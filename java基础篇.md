...
![img](http://note.youdao.com/src/34DC88CAD16F4CF486235871CFFA0B12)
...
![img](http://note.youdao.com/src/B390078514114C50BB21A523569556FE)
...
![img](http://note.youdao.com/src/63AC7490B3B8457E90D675BEBF9845C5)
...
String json = "{name:\"jason\",father:\"jason\",age:18}";	//name:"jason"//age:18//\"\\w+\" 字符串属性Pattern p = Pattern.compile("\\w+:(\"\\w+\"|\\d*)");Matcher m = p.matcher(json);while(m.find()){	String text = m.group();	int dotPos= text.indexOf(":");	String key = text.substring(0, dotPos);	String value = text.substring(dotPos+1, text.length());	//替换字符串的开始结束的双引号	value = value.replaceAll("^\\\"|\\\"$", "");	System.out.println(key);	System.out.println(value);}
