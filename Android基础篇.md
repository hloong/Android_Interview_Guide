...
- singleTask 栈内复用模式，检查整个栈内如果有那就复用，并回调onNewIntent方法；此模式启动 Activity A，系统首先会寻找是

否存在 A 想要的任务栈，如果不存在，就会重新创建一个任务栈，然后把创建好 A 的实例放到栈中；
...
![img](http://note.youdao.com/src/A46F7A75045246E380DFFE3C36B463EC)
...
- 设置 Activity 的android:configChanges="orientation|keyboardHidden|screenSize"时，机型测试通过，切屏不会重新调用各个生命周期，只会
...
![img](http://note.youdao.com/src/9B227ADF696D4166BCD7DE956EAEAB18)
...
![img](http://note.youdao.com/src/DCB2DBD4EA344A4D9666A77464E1E008)
...
![img](http://note.youdao.com/src/D8706BA3991D42D9A194F1D53B75BA66)
...
AnimatorSet.play(Animator anim)   ：播放当前动画AnimatorSet.after(long delay)   ：将现有动画延迟x毫秒后执行AnimatorSet.with(Animator anim)   ：将现有动画和传入的动画同时执行AnimatorSet.after(Animator anim)   ：将现有动画插入到传入的动画之后执行AnimatorSet.before(Animator anim) ：  将现有动画插入到传入的动画之前执行 
...
![img](http://note.youdao.com/src/116FCB22E4164208985EE60E7DA9452D)
...
![img](http://note.youdao.com/src/563817C6233F45418978E39AE81AF5C9)
...
扩展：<https://blog.csdn.net/qq_26626709/article/details/52206067>）
...
![img](http://note.youdao.com/src/DE155C620806410C984EBEF082558629)

怎么调用？（SM = ServiceManager）

先去serviceManager注册一个方法，然后通过binder驱动来转换成代理对象返回给客户端完成注册。

![img](http://note.youdao.com/src/714EF537EB844508BFB82E23EEFE2409)
...
![img](http://note.youdao.com/src/8A5F53B3010944C8985803F61F88EABA)
...
![img](http://note.youdao.com/src/B15007F9CB0F4BB985FBACC5BB5BC504)
...
if(widthMeasureSpec == MeasureSpec.AT_MOST && heightMeasureSpec == MeasureSpec.AT_MOST){  setMeasuredDimension(200,200); // wrap_content情况下要设置一个默认值，最终的值需要计算得到刚好包裹内容的宽高值}else if(widthMeasureSpec == MeasureSpec.AT_MOST){    setMeasuredDimension(200,heightMeasureSpec );}else if(heightMeasureSpec == MeasureSpec.AT_MOST){    setMeasuredDimension(heightMeasureSpec ,200);}
...
![img](http://note.youdao.com/src/391A4651DAC4417299A4B0EB34327FE7)
...
​    public void postInvalidate() {        postInvalidateDelayed(0);    }    public void postInvalidateDelayed(long delayMilliseconds) {        // We try only with the AttachInfo because there's no point in invalidating        // if we are not attached to our window        final AttachInfo attachInfo = mAttachInfo;        if (attachInfo != null) {            attachInfo.mViewRootImpl.dispatchInvalidateDelayed(this, delayMilliseconds);        }    }    public void dispatchInvalidateDelayed(View view, long delayMilliseconds) {        Message msg = mHandler.obtainMessage(MSG_INVALIDATE, view);        mHandler.sendMessageDelayed(msg, delayMilliseconds);        //这里的mHandler是ViewRootHandler实例，在该Handler的handleMessage方法中调用了view.invalidate()方法。    }    public void handleMessage(Message msg) {            switch (msg.what) {            case MSG_INVALIDATE:                ((View) msg.obj).invalidate();                break;            }     }
